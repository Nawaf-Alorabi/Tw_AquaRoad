...
def save_report_to_db(source, status):
    ...
    c.execute("INSERT INTO reports (date, time, source, status) VALUES (?, ?, ?, ?)",
              (date_str, time_str, source, status))
    conn.commit()
    conn.close()
...
with col_info:
    ...
    # Location mapping based on camera selection
    if source_option == "Camera #402":
        location_name = "Al-Hada District, Riyadh"
        coordinates = "24.71°N, 46.67°E"
    elif source_option == "Camera #105":
        location_name = "Al-Malqa District, Riyadh"
        coordinates = "24.82°N, 46.61°E"
    else:
        location_name = "Test Zone, Riyadh"
        coordinates = "00.00°N, 00.00°E"

    # Show changing data in card
    st.markdown(f"""
...
while cap.isOpened():
    ...
    if len(results[0].boxes) > 0:
        ...
        # Send alert and save every 10 minutes to avoid redundant data
        if current_time - st.session_state.last_alert_time > 600:
            ...
            # 2. Save Data to DB Automatically (Status: Detected)
            save_report_to_db(source_option, "Detected")
            
            st.session_state.last_alert_time = current_time
            st.toast(f'Alert Saved & Sent: {source_option}')
            
            # Update UI
            alert_log_placeholder.markdown(f"""
